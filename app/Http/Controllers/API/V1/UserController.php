...
public function agent_login(Request $request){
      ...
      if($request->api_key){
          ...
          if($user){
            ...
            if($remaining_value > 0){
                $your_emi = round(($avg_sal*$remaining_value)/100, 0);
                $your_limit = round($your_emi*20, 0);
                $eligible = 1;
            } else {
                $your_limit = 0;
                $your_emi = 0;
    ...
    public function emirates_id_duplicacy(Request $request){
      
      if($request->login == 0){
        $user = User::where('eid_number', $request->eid_number)->count();
        if($user == 0){
        return response()->json(['success' => true, 'status' => 200, 'message' => 'Valid Emirates ID']);
        } else {
        return response()->json(['success' => false, 'status' => 201, 'message' => 'Emirates ID is already exist. Try again']);
        }
      } else {
        $user = User::where('api_key', '!=', $request->api_key)->where('eid_number', $request->eid_number)->count();
         if($user == 0){
        return response()->json(['success' => true, 'status' => 200, 'message' => 'Valid Emirates ID']);
        } else {
        return response()->json(['success' => false, 'status' => 201, 'message' => 'Emirates ID is already exist. Try again']);
